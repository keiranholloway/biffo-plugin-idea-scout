...
> new repository, rename `idea_scout` throughout to your plugin's name,
...
│   ├── plugin.py            # IdeaScoutPlugin(BiffoPluginBase) — the reference implementation
...
API); this template's `idea_scout` is deliberately the smallest version of
the same shape: one table (`idea_scout_widgets`), four routes, one `on_install`
