"""Tests for IdeaScoutPlugin: manifest loading, lifecycle hooks, and the
...
from idea_scout.plugin import _DEFAULT_WIDGET, IdeaScoutPlugin


def _make_plugin() -> tuple[IdeaScoutPlugin, FakeCoreApi]:
    fake = FakeCoreApi()
    plugin = IdeaScoutPlugin(api=fake.client())
    return plugin, fake
...
class TestManifestLoading:
    def test_manifest_name_and_version(self) -> None:
        ...
        assert plugin.manifest.name == "idea-scout"
    ...
    def test_manifest_declares_the_widgets_table(self) -> None:
        ...
        assert table_names == {"idea_scout_widgets"}

    def test_register_returns_manifest_registration(self) -> None:
        plugin, _ = _make_plugin()
        registration = plugin.register()
        assert registration["name"] == "idea-scout"
        assert {t["name"] for t in registration["tables"]} == {"idea_scout_widgets"}
